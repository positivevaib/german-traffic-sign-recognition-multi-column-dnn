...
parser.add_argument('-l',
                    '--load',
                    action='store_true',
                    help='load pre-trained model parameters')
parser.add_argument('-m',
                    '--model',
                    type=str,
                    default=os.getcwd(),
                    help='absolute path to model parameters')
...
training_set = data.load_dataset(args.data,
                                 dataset='training',
...
for net_name in nets.keys():
    ...
    if args.load:
        print(net_name)
        net.load_state_dict(
            torch.load(os.path.join(args.model, (net_name + '.pth')),
                       map_location=device))
    else:
        ...
        for epoch in range(total_epochs):
            ...
            for batch_idx, data in enumerate(training_loader):
                ...
                for i in range(len(inputs)):
                    ...
                    if perturbation == 1:
                        ...
                        image = transforms.RandomAffine(
                            degrees=0,
                            translate=(0.1, 0.1),
                            resample=PIL.Image.BILINEAR)(image)
                        inputs[i] = transforms.ToTensor()(image)
                    elif perturbation == 2:
                        image = transforms.ToPILImage()(inputs[i])
                        image = transforms.RandomResizedCrop(size=48,
                                                             ratio=(1,
                                                                    1))(image)
                        inputs[i] = transforms.ToTensor()(image)
                    elif perturbation == 3:
                        image = transforms.ToPILImage()(inputs[i])
                        image = transforms.RandomRotation(
                            degrees=5, resample=PIL.Image.BILINEAR)(image)
                        inputs[i] = transforms.ToTensor()(image)
                    else:
                        continue
                ...
                if epoch == 0 and batch_idx == 0:
                    net.to('cpu')
                    with torch.no_grad():
                        validation_inputs, validation_labels = next(
                            iter(validation_loader))
                        validation_loss_prev = criterion(
                            net(validation_inputs), validation_labels)
                    net.to(device)

                    loss_file.write(
                        str(epoch) + ',' + str(running_loss) + ',' +
                        str(validation_loss_prev.item()) + '\n')

                    print('initial training loss:', running_loss)
                    print('initial validation loss:',
                          validation_loss_prev.item())
            ...
            with torch.no_grad():
                validation_inputs, validation_labels = next(
                    iter(validation_loader))
                validation_loss = criterion(net(validation_inputs),
                                            validation_labels)
            net.to(device)

            loss_file.write(
                str(epoch + 1) + ',' + str(running_loss / batch_idx + 1) +
                ',' + str(validation_loss.item()) + '\n')
            ...
            torch.save(net.state_dict(),
                       os.path.join(args.model, (net_name + '.pth')))
...
with torch.no_grad():
    mcdnn_outputs = torch.zeros(
        (next(iter(original_test_loader))[0].size()[0], 43))
